...
#             X_reduced        = X_train[np.array(index_samples),:][:,np.array(columns_to_check)]
            ...
            RC_array         = alpha*ones_P[len(columns_to_check)-1] - np.abs( np.dot(X_reduced.T, RC_aux) ) if not is_sparse else alpha*ones_P[len(columns_to_check)-1] - np.abs( X_reduced.T.dot( RC_aux) )
...
#             X_reduced            = X_train[:, np.array(index_columns)][np.array(constraint_to_check), :]
            ...
            # if violated_constraints.shape[0] > 125:
            #     indexes = np.argsort(RC_array_cons)[:125]
            #     violated_constraints = np.array(constraint_to_check)[indexes]
    ...
    #-------ADD CONSTRAINTS
        #---Add the constraints with negative reduced costs
            ...
            print('Number of violated colums ..... : ', n_columns_to_add)
            # exit()
...
#             model.update()
    ...
    print('Total number of variables added: ', len(index_columns))
    print('Total number of constraints added: ', len(index_samples))
